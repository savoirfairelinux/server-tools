...
class IrModel(orm.Model):
    ...
    def _get_linked_models(self, models, required):
        ...
        for model in models:
            ...
            for field_name, field in self.pool.get(model)._columns.iteritems():
                if field._type in ('many2one', 'many2many') and \
                        (not hasattr(field, 'store') or field.store) \
                        and field._obj in models and field._obj != model and \
                                field.required == required:
                    linked_models[model].setdefault(
                        field._obj, []).append('%s:id' % field_name
                    )
    ...
    def get_model_graph(self, cr, uid, ids, context=None):
        ...
        models = [
            ...
            and hasattr(self.pool.get(model.model), 'get_fields_to_export')
        ]
        ...
        for index, (model, model_fields) in enumerate(ordered_models):
            ...
            field_to_export = self.pool.get(model).get_fields_to_export()
            ordered_models[index] = (
                model,
                list(set(field_to_export) - set(not_required_link_fields))
            )

        for model in not_required_linked_models:
            for linked_model, link_fields in \
                    not_required_linked_models[model].iteritems():
                index = get_index(ordered_models, model)
                if index > get_index(ordered_models, linked_model):
                    ordered_models[index] = (
                        model,
                        ordered_models[index][1] + link_fields
                    )
                else:
